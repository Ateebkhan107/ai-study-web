...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { label: "Total Questions", value: stats.totalQuestions, icon: List, color: "text-indigo-500", bg: "bg-indigo-50 dark:bg-indigo-900/20" },
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {statCards.map((card, i) => (
          <div key={i} className="bg-[var(--card)] dark:bg-[var(--surface)] border border-gray-200 dark:border-[var(--border-subtle)] rounded-2xl p-6 shadow-sm">
      ...
      <div className="bg-[var(--card)] dark:bg-[var(--surface)] border border-gray-200 dark:border-[var(--border-subtle)] rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link href="/admin/import" className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 dark:border-[var(--border-subtle)] rounded-xl hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/10 transition-all text-center gap-2 group">
            <UploadCloud className="w-8 h-8 text-gray-400 group-hover:text-indigo-500" />
            <span className="font-bold text-sm">Import CSV</span>
          </Link>
          <Link href="/admin/exams" className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 dark:border-[var(--border-subtle)] rounded-xl hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all text-center gap-2 group">
            <FileText className="w-8 h-8 text-gray-400 group-hover:text-blue-500" />
            <span className="font-bold text-sm">Manage Exams</span>
          </Link>
          <Link href="/admin/images" className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 dark:border-[var(--border-subtle)] rounded-xl hover:border-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/10 transition-all text-center gap-2 group">
            <ImageIcon className="w-8 h-8 text-gray-400 group-hover:text-amber-500" />
            <span className="font-bold text-sm">Missing Images</span>
          </Link>
          <Link href="/admin/notifications" className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 dark:border-[var(--border-subtle)] rounded-xl hover:border-green-500 hover:bg-green-50 dark:hover:bg-green-900/10 transition-all text-center gap-2 group">
