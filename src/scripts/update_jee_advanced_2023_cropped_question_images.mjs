...
const CLEAN_ROOT = path.join(process.cwd(), "tmp", "jee-advanced-2023-clean-question-images");
const reportPath = path.join(process.cwd(), "tmp", "jee-advanced-2023-cropped-image-report.json");
...
function paperMeta(paperCode) {
  const match = paperCode.match(/^JEE-ADV-23-P([12])$/);
...
for (const paperCode of PAPER_CODES) {
  ...
  for (const row of rows) {
    ...
    const objectPath = `jee-advanced/2023-cleaned/paper-${paperNumber}/q${number}.jpg`;
