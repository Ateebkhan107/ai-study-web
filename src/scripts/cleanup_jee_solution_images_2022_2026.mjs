...
const REPORT_PATH = "tmp/jee-solution-image-cleanup-2022-2026.json";
...
const targets = rows.filter((row) => (
  String(row.exam).toUpperCase().startsWith("JEE")
  && Number(row.year) >= 2022
  && Number(row.year) <= 2026
...
const report = {
  ...
  scope: { exam: "JEE", years: [2022, 2023, 2024, 2025, 2026] },
  scannedQuestions: rows.length,
  targetRows: targets.length,
  targetRowsByYear: byYear,
...
if (APPLY && targets.length > 0) {
  ...
  for (let index = 0; index < deletablePaths.length; index += 100) {
    const chunk = deletablePaths.slice(index, index + 100);
  ...
  if (error) throw error;
  report.remainingTargetReferences = count;
  if (count !== 0) throw new Error(`${count} target explanation-image references remain`);
}
