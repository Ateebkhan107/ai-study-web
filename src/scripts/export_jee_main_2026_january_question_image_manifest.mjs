...
for (const paperCode of PAPER_CODES) {
  ...
  if (rows.length !== 75) {
    throw new Error(`${paperCode}: expected 75 rows, found ${rows.length}`);
  }

  manifest.push(...rows);
}

const outputPath = path.join(process.cwd(), "tmp", "jee-main-2026-january-question-image-manifest.json");
