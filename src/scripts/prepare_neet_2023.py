...
OPTION_RE = re.compile(r"(?i)(?:^|\s+)\(([a-d1-4])\)\s")
...
def parse_questions(pdf_path: Path) -> list[dict]:
    ...
    if numbers != list(range(1, 201)):
        raise ValueError(f"Expected question numbers 1-200, found {numbers[:5]} ... {numbers[-5:]}")
        ...
        questions.append(
            {
                ...
                "exam_type": "NEET UG",
                "year": 2023,
                "attempt": "NEET UG 2023",
                "shift": "Single Shift",
                "paper_code": "NEET 2023",
                "subject": subject,
...
def render_question_images(pdf_path: Path, output_dir: Path) -> dict[int, str]:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for number in range(1, 201):
            ...
            path = image_dir / f"neet-ug-2023-question-{number:03d}.png"
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("Usage: prepare_neet_2023.py INPUT_PDF OUTPUT_DIRECTORY")
    ...
    manifest_path = output_dir / "neet-ug-2023-manifest.json"
