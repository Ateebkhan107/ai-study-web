 ...
 * source-faithful question visuals first, then inserts the 180 question rows.
 * Run with: node src/scripts/import_neet_2025.mjs tmp/neet-ug-2025/neet-ug-2025-manifest.json
 */
...
if (!manifestPath) {
  throw new Error("Usage: node src/scripts/import_neet_2025.mjs MANIFEST_PATH");
}
...
if (count > 0) {
  throw new Error(`Import stopped: ${count} NEET 2025 Narmada 48 records already exist.`);
}
...
for (let start = 0; start < manifest.length; start += 12) {
  ...
  const objectPath = `neet-ug-2025/narmada-48/question-${String(item.number).padStart(3, "0")}.png`;
...
if (importedCount !== 180) {
  throw new Error(`Import verification failed: expected 180 rows, found ${importedCount}`);
}
