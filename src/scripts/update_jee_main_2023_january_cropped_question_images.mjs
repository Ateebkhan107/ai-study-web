...
const PAPER_CODES = [
  "JEE-MAIN-23-24JAN-S1",
  "JEE-MAIN-23-24JAN-S2",
  "JEE-MAIN-23-25JAN-S1",
  "JEE-MAIN-23-25JAN-S2",
  "JEE-MAIN-23-29JAN-S1",
  "JEE-MAIN-23-29JAN-S2",
  "JEE-MAIN-23-30JAN-S1",
  "JEE-MAIN-23-30JAN-S2",
  "JEE-MAIN-23-31JAN-S1",
  "JEE-MAIN-23-31JAN-S2",
];
const requestedPapers = process.argv.slice(2);
const activePaperCodes = requestedPapers.length ? requestedPapers : PAPER_CODES;

const CLEAN_ROOT = path.join(process.cwd(), "tmp", "jee-main-2023-january-clean-question-images");
const reportPath = path.join(process.cwd(), "tmp", "jee-main-2023-january-cropped-image-report.json");
...
function paperSlug(paperCode) {
  const match = paperCode.match(/^JEE-MAIN-23-(\d{2}JAN)-S([12])$/);
...
for (const paperCode of activePaperCodes) {
  ...
  for (const row of rows) {
    ...
    const objectPath = `jee-main-2023-january/${paperSlug(paperCode)}/q${number}.png`;
