...
const PAPER_CODES = ["JEE-ADV-24-P1", "JEE-ADV-24-P2"];
const CLEAN_ROOT = path.join(process.cwd(), "tmp", "jee-advanced-2024-clean-question-images");
const reportPath = path.join(process.cwd(), "tmp", "jee-advanced-2024-cropped-image-report.json");
...
function paperMeta(paperCode) {
  const match = paperCode.match(/^JEE-ADV-24-P([12])$/);
...
for (const paperCode of PAPER_CODES) {
  ...
  if (rows.length !== 51) throw new Error(`${paperCode}: expected 51 rows, found ${rows.length}`);
  ...
  for (const row of rows) {
    ...
    const objectPath = `jee-advanced/2024-cleaned/paper-${paperNumber}/q${number}.jpg`;
