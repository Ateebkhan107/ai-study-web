 ...
 * Run with: node src/scripts/import_neet_2023.mjs tmp/neet-ug-2023/neet-ug-2023-manifest.json
 */
...
if (!manifestPath) {
  throw new Error("Usage: node src/scripts/import_neet_2023.mjs MANIFEST_PATH");
}
...
const { count, error: countError } = await supabase
  ...
  .eq("exam", "NEET")
  .eq("year", 2023)
  .eq("paper_code", "NEET 2023");

if (countError) throw countError;
if (count > 0) {
  throw new Error(`Import stopped: ${count} NEET 2023 records already exist.`);
}
...
for (let start = 0; start < manifest.length; start += 12) {
  ...
  const objectPath = `neet-ug-2023/question-${String(item.number).padStart(3, "0")}.png`;
...
const { data: pkgData, error: pkgErr } = await supabase
  ...
  .insert([{ name: "NEET 2023 (Auto Import)" }])
  .select("id")
  .single();
