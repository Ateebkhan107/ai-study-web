...
const PAPER_CODE = "JEE-MAIN-24-31JAN-S1";
const PDF_PATH = path.join(process.cwd(), "tmp/pdfs/source-a.pdf");
const WORK_DIR = path.join(process.cwd(), "tmp/jee-main-2024-31jan-s1-clean-repair");
...
const shiftVisualCrops = {
  34: { source: 4, page: 1, box: [170, 960, 1350, 1540], note: "velocity-time graph options" },
  35: { source: 5, page: 1, box: [170, 1800, 1030, 2035], note: "doubly inclined plane diagram" },
  40: { source: 10, page: 2, box: [170, 1210, 690, 1510], note: "isobaric process graph" },
  44: { source: 14, page: 3, box: [170, 390, 850, 730], note: "wire in magnetic field" },
  50: { source: 20, page: 4, box: [170, 260, 760, 450], note: "logic gate circuit" },
  56: { source: 26, page: 4, box: [170, 1490, 1250, 1730], note: "resistor network" },
  89: { source: 59, page: 9, box: [170, 240, 1250, 650], note: "organic reaction product P" },
};

const shiftManualText = {
  3: {
    question: "Question 3: The sum of the series $\\frac1{1-3\\cdot1^2+1^4}+\\frac1{1-3\\cdot2^2+2^4}+\\frac1{1-3\\cdot3^2+3^4}+\\cdots$ up to 10 terms is:",
    options: ["$\\frac{45}{109}$", "$-\\frac{45}{109}$", "$\\frac{55}{109}$", "$-\\frac{55}{109}$"],
  },
  14: {
    question: "Question 14: The area of the region $\\{(x,y): y^2\\le4x, x<4, \\frac{x}{x^2-3x-4}>0, x\\ne3\\}$ is:",
    options: ["$\\frac{16}{3}$", "$\\frac{64}{3}$", "$\\frac83$", "$\\frac{32}{3}$"],
  },
  19: {
    question: "Question 19: Two marbles are drawn in succession from a box containing 10 red, 30 white, 20 blue and 15 orange marbles, with replacement being made after each drawing. Then the probability that the first drawn marble is red and second drawn marble is white is:",
    options: ["$\\frac2{25}$", "$\\frac4{25}$", "$\\frac23$", "$\\frac4{75}$"],
  },
  20: {
    question: "Question 20: Three rotten apples are accidentally mixed with fifteen good apples. Assuming the random variable x to be the number of rotten apples in a draw of two apples, the variance of x is:",
    options: ["$\\frac{37}{153}$", "$\\frac{57}{153}$", "$\\frac{47}{153}$", "$\\frac{40}{153}$"],
  },
  30: {
    question: "Question 30: Let Q and R be the feet of perpendiculars from the point $P(a,a,a)$ on the lines $x=y, z=1$ and $x=-y, z=-1$ respectively. If $\\angle QPR$ is a right angle, then $12a^2$ is equal to ______.",
    options: [],
  },
  34: {
    question: "Question 34: A small steel ball is dropped into a long cylinder containing glycerine. Which one of the following is the correct representation of the velocity-time graph for the transit of the ball?",
    options: ["See figure option 1", "See figure option 2", "See figure option 3", "See figure option 4"],
  },
  36: {
    question: "Question 36: A coin is placed on a disc. The coefficient of friction between the coin and the disc is $\\mu$. If the distance of the coin from the center of the disc is r, the maximum angular velocity which can be given to the disc, so that the coin does not slip away, is:",
    options: ["$\\sqrt{\\frac{\\mu g}{r}}$", "$\\sqrt{\\frac{rg}{\\mu}}$", "$\\frac{\\mu g}{r}$", "$\\frac{\\mu}{rg}$"],
  },
  38: {
    question: "Question 38: Four identical particles of mass m are kept at the four corners of a square. If the gravitational force exerted on one of the masses by the other masses is $\\frac{(2\\sqrt2+1)Gm^2}{32L^2}$, the length of the sides of the square is:",
    options: ["L", "4L", "3L", "2L"],
  },
  43: {
    question: "Question 43: Two charges q and 3q are separated by a distance r in air. At a distance x from charge q, the resultant electric field is zero. The value of x is:",
    options: ["$\\frac{r}{1+\\sqrt3}$", "$\\frac{3r}{1+\\sqrt3}$", "$\\frac{r}{\\sqrt3-1}$", "$\\frac{\\sqrt3r}{1+\\sqrt3}$"],
  },
};
...
function normalizeText(value) {
  return String(value || "")
    ...
    .replace(/J\s*EE Main 2024 \(31 Jan Shift 1\)\s*JEE Main Previous Year Paper\s*Question Paper\s*MathonGo/g, " ")
    .replace(/\u0000/g, "")
    .replace(/\s+/g, " ")
...
async function uploadImage(questionNumber) {
  ...
  const objectPath = `jee-main-2024/31jan-shift-1-required/${filename}`;
