...
const CLEAN_ROOT = path.join(process.cwd(), "tmp", "jee-main-2025-january-clean-question-images");
const reportPath = path.join(process.cwd(), "tmp", "jee-main-2025-january-cropped-image-report.json");
const report = {
...
function paperSlug(paperCode) {
  const match = paperCode.match(/^JEE-MAIN-25-(\d{2})JAN-S([12])$/);
  if (!match) throw new Error(`Unexpected paper code: ${paperCode}`);
  const [, day, shift] = match;
  return `${day}-jan-shift-${shift}-cleaned`;
}
