...
const CLEAN_ROOT = path.join(process.cwd(), "tmp", "jee-main-2024-april-clean-question-images");
const reportPath = path.join(process.cwd(), "tmp", "jee-main-2024-april-cropped-image-report.json");
...
function paperSlug(paperCode) {
  const match = paperCode.match(/^JEE-MAIN-24-(\d{2}APR)-S([12])$/);
...
for (const paperCode of activePaperCodes) {
  ...
  for (const row of rows) {
    ...
    const objectPath = `jee-main-2024-april/${paperSlug(paperCode)}/q${number}.png`;
