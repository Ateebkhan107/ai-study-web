...
ANSWER=re.compile(r"(?im)^\s*Answer\s*\(\s*([1-4](?:\s*&\s*[1-4])?)\s*\*?\s*\)")
...
def main()->None:
 ...
 text="\n".join(streams);text=re.sub(r"(?m)^(\s*51)\s+(?=Concentration)",r"\1. ",text);text=text.replace("\nprotein with 333 amino acids,", "\n130. A gene has 1000 bases and codes for a protein with 333 amino acids,").replace("\n(d) (b), (c) & (d)","\n(4) (b), (c) & (d)");matches=[m for m in QUESTION.finditer(text) if 1<=int(m.group(1))<=180];chosen=[];cursor=0
 ...
 for i,start in enumerate(chosen):
  number=i+1;end=chosen[i+1].start() if i<179 else len(text);block=text[start.end():end];answer=ANSWER.search(block)
  ...
  accepted=["abcd"[int(x)-1] for x in re.findall(r"[1-4]",answer.group(1))];key=accepted[0];s=subject(number)
  explanation=compact(block[answer.end():]);rows.append(dict(number=number,exam="NEET",exam_type="NEET UG",year=2017,attempt="NEET UG 2017",shift="Single Shift",paper_code="NEET 2017 Code A",subject=s,chapter=f"NEET 2017 {s}",question_type="MCQ",question=question,option_a=options[0],option_b=options[1],option_c=options[2],option_d=options[3],correct_option=key,accepted_options=accepted,explanation=explanation,marks_positive=4,marks_negative=-1))
