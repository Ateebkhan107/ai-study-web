...
async function publishPaper(paperCode, attempt, shift, questions) {
  ...
  for (const q of questions) {
    ...
    const optA = q.option_a ?? (q.options ? (Array.isArray(q.options) ? q.options[0] : q.options.a) : "") ?? "";
    const optB = q.option_b ?? (q.options ? (Array.isArray(q.options) ? q.options[1] : q.options.b) : "") ?? "";
    const optC = q.option_c ?? (q.options ? (Array.isArray(q.options) ? q.options[2] : q.options.c) : "") ?? "";
    const optD = q.option_d ?? (q.options ? (Array.isArray(q.options) ? q.options[3] : q.options.d) : "") ?? "";
...
// 05APR-S1
const s5_maths = JSON.parse(await fs.readFile("tmp/05apr_s1_maths.json", "utf8"));
const s5_physics = JSON.parse(await fs.readFile("tmp/05apr_s1_physics.json", "utf8"));
const s5_chem = JSON.parse(await fs.readFile("tmp/05apr_s1_chem.json", "utf8"));
await publishPaper("JEE-MAIN-26-05APR-S1", "5 Apr", "Shift 1", [...s5_maths, ...s5_physics, ...s5_chem]);
