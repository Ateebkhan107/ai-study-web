...
export default function AnalyticsPageClient({
  initialStats = null,
  initialTrack = "JEE",
  initialAdvancedAnalyticsAllowed = false,
}) {
  const [stats, setStats] = useState(initialStats);
  const [loading, setLoading] = useState(false);
  const [activeTrack, setActiveTrack] = useState(initialTrack);
  const [activeTab, setActiveTab] = useState("overview");
  const [advancedAnalyticsAllowed, setAdvancedAnalyticsAllowed] = useState(initialAdvancedAnalyticsAllowed);
  const analyticsCacheRef = useRef(new Map(initialStats ? [[initialStats.track || initialTrack, initialStats]] : []));
  ...
  useEffect(() => {
    async function syncTrackFromCookie() {
      ...
      if (cookieTrack !== activeTrack) setActiveTrack(cookieTrack);
    }
  ...
  useEffect(() => {
    ...
    async function loadAnalytics() {
      ...
      if (!initialStats && !initialAdvancedAnalyticsAllowed) {
        setLoading(false);
        return;
      }
      ...
      try {
        setLoading(true);
        const data = await getUserAnalytics(null, activeTrack, { signal: controller.signal });
  ...
  }, [activeTab, activeTrack, initialAdvancedAnalyticsAllowed, initialStats]);
