    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 dark:bg-[var(--surface)]/50 p-4 sm:p-5 rounded-2xl border dark:border-[var(--border-subtle)] space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <select value={filters.exam} onChange={e => handleFilterChange('exam', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm">
            <option value="">All Exams</option>
            <option value="JEE">JEE</option>
            <option value="NEET">NEET</option>
          </select>
          <input placeholder="Exam Type" value={filters.exam_type} onChange={e => handleFilterChange('exam_type', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm" />
          <input type="number" placeholder="Year" value={filters.year} onChange={e => handleFilterChange('year', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm" />
          <input type="date" value={filters.attempt} onChange={e => handleFilterChange('attempt', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm text-gray-500" title="Exam Date" />
          <input placeholder="Shift" value={filters.shift} onChange={e => handleFilterChange('shift', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm" />
          <input placeholder="Subject" value={filters.subject} onChange={e => handleFilterChange('subject', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm" />
          <input placeholder="Chapter" value={filters.chapter} onChange={e => handleFilterChange('chapter', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm" />
          <select value={filters.question_type} onChange={e => handleFilterChange('question_type', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm">
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <select value={filters.image_status} onChange={e => handleFilterChange('image_status', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 w-full bg-transparent text-sm">
          ...
          <div className="relative">
            ...
            <input placeholder="Search ID, text, subject..." value={filters.search} onChange={e => handleFilterChange('search', e.target.value)} className="border dark:border-[var(--border)] rounded-xl p-2 pl-9 w-full bg-transparent text-sm" />
          </div>
        </div>
      </div>

      {/* TABLE */}
      <div className="border dark:border-[var(--border-subtle)] rounded-2xl overflow-hidden bg-[var(--card)] dark:bg-[#0A0D1A]">
        <div className="overflow-x-auto max-h-[600px] relative">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-gray-50 dark:bg-[var(--surface)] sticky top-0 z-10 text-gray-500 uppercase text-xs font-semibold">
              <tr>
                ...
                <th className="p-4 sticky right-0 bg-gray-50 dark:bg-[var(--surface)] z-20">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              {!loading && questions.map(q => (
                <tr key={q.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                  ...
                  <td className="p-4">
                    <span className="bg-gray-100 dark:bg-[var(--surface-elevated)] px-2 py-1 rounded text-xs">{q.question_type}</span>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 sticky right-0 bg-[var(--card)] dark:bg-[#0A0D1A] group-hover:bg-gray-50 dark:group-hover:bg-gray-800/50 z-10 transition-colors">
        ...
        {/* PAGINATION */}
        <div className="p-4 border-t dark:border-[var(--border-subtle)] flex flex-col gap-3 bg-gray-50 dark:bg-[var(--surface)]/50 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-sm text-gray-500">Total: {totalCount}</span>
            <select value={limit} onChange={e => { setLimit(Number(e.target.value)); setPage(1); }} className="border dark:border-[var(--border)] bg-transparent rounded p-1 text-sm">
      ...
      {isDrawerOpen && editingQuestion && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-[800px] bg-[var(--card)] dark:bg-[var(--background)] h-full shadow-2xl flex flex-col animate-slideLeft border-l dark:border-[var(--border-subtle)]">
            {/* Drawer Header */}
            <div className="p-4 sm:p-5 border-b dark:border-[var(--border-subtle)] flex items-start justify-between gap-3 sticky top-0 bg-[var(--card)] dark:bg-[var(--background)] z-10">
              ...
              <div className="flex shrink-0 items-center gap-2 sm:gap-3">
                ...
                <div className="w-px h-6 bg-gray-200 dark:bg-[var(--surface-elevated)] mx-2"></div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 sm:space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea value={editingQuestion.question || ""} onChange={e => setEditingQuestion({...editingQuestion, question: e.target.value})} className="w-full border dark:border-[var(--border)] rounded-lg p-3 bg-transparent h-24 text-sm" />
                ...
                {editingQuestion.question_type !== 'Numerical' && (
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {['a','b','c','d'].map(opt => (
                      <div key={opt} className="space-y-2 p-4 border dark:border-[var(--border-subtle)] rounded-lg bg-gray-50 dark:bg-[var(--surface)]/30">
                        <label className="text-sm font-bold uppercase">Option {opt}</label>
                        <textarea value={editingQuestion[`option_${opt}`] || ""} onChange={e => setEditingQuestion({...editingQuestion, [`option_${opt}`]: e.target.value})} className="w-full border dark:border-[var(--border)] rounded-lg p-2 bg-transparent text-sm h-16" />
                ...
                <div className="space-y-2">
                  ...
                  <textarea value={editingQuestion.explanation || ""} onChange={e => setEditingQuestion({...editingQuestion, explanation: e.target.value})} className="w-full border dark:border-[var(--border)] rounded-lg p-3 bg-transparent h-24 text-sm" />
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            ...
            {/* Drawer Footer */}
            <div className="p-4 sm:p-5 border-t dark:border-[var(--border-subtle)] bg-gray-50 dark:bg-[var(--surface)] sticky bottom-0 z-10 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <button onClick={closeDrawer} className="min-h-11 px-5 py-2 rounded-lg font-bold text-sm">Cancel</button>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <button onClick={() => saveQuestion(false)} disabled={saving} className="min-h-11 justify-center bg-gray-200 dark:bg-[var(--surface-elevated)] px-5 py-2 rounded-lg font-bold text-sm disabled:opacity-50 flex items-center gap-2">
