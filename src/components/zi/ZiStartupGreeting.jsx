...
export default function ZiStartupGreeting({
  ...
  return (
    <div
      className={`zi-startup-greeting fixed right-4 z-[54] flex max-w-[min(20rem,calc(100vw-2rem))] items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--card)]/95 px-3 py-3 text-white shadow-2xl backdrop-blur-xl transition-opacity duration-500 motion-reduce:transition-opacity sm:right-7 ${
        isVisible ? "opacity-100" : "opacity-0"
      } bottom-[calc(12.75rem+env(safe-area-inset-bottom))] sm:bottom-[calc(11.25rem+env(safe-area-inset-bottom))]`}
      aria-live="polite"
      aria-atomic="true"
    >
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-brand/20 bg-brand/10 shadow-sm">
        <Logo size={20} showText={false} forceDark />
      </div>
      <div className="min-w-0">
        <p className="truncate font-display text-base font-black leading-5 text-white">
          {greetingLine}
        </p>
        <p
          className={`mt-0.5 truncate text-xs font-bold text-brand transition-opacity duration-500 ${
