...
function getXpData(profile) {
  if (!profile) return null;

  return {
    xp: profile.xp || 0,
    level: profile.level || 1,
    badge: profile.badge || "Explorer",
...
export default function ProfilePageClient({ initialProfile = null }) {
  ...
  const [xpData, setXpData] = useState(() => getXpData(initialProfile));
  const [rank, setRank] = useState(initialProfile?.rank ?? null);

  const [dynamicBadges, setDynamicBadges] = useState([]);
  const [badgesLoading, setBadgesLoading] = useState(true);
  const [loading, setLoading] = useState(!initialProfile);
  const [editing, setEditing] = useState(false);
  const [editName, setEditName] = useState(initialProfile?.full_name || "Student");
  ...
  const [editExam, setEditExam] = useState(initialProfile?.current_track?.toUpperCase() || "JEE");
  const [editYear, setEditYear] = useState(initialProfile?.target_year || 2026);
  const [saved, setSaved] = useState(false);

  const currentYear = new Date().getFullYear();
  const years = Array.from({ length: 4 }, (_, i) => currentYear + i);

  // LOAD PROFILE: server-rendered first load already includes this payload.
  useEffect(() => {
    ...
    fetchLiveProfile();
  }, [clerkUser?.fullName, clerkUser?.id, initialProfile]);
  ...
  const activeUser = {
    ...
    email: clerkUser?.primaryEmailAddress?.emailAddress || user?.email || "No email",
